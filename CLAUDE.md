...
| 자작캠핑카 | 포스트별 서브폴더 | `YYYYMMDD_설명.확장자` | `20260611_ceiling_louver.jpg` |
...
포스트 커밋 전 아래 규칙으로 맞춤법/띄어쓰기를 점검한다 (코드 블록, HTML 속성 값, wikilink `[[...]]`, 마크다운 링크, frontmatter, URL은 점검 대상에서 제외):

- 인용부호(`'...'`, `"..."`, `` `...` ``) 뒤에 오는 말이 인용격 조사(`라고`, `이라고`, `란`, `이란` 등)이면 앞말에 붙여 쓴다. 대화 인용인지 여부와 무관하게 항상 붙임.
- 인용부호 뒤에 `하고`가 오는 경우는 '하다'의 활용형(독립된 서술어)이므로 띄어 쓴다.
- `~구요`(그렇구요, 있구요, 크구요 등)는 `~고요`가 표준形이므로 통일한다 (`~더라구요`는 예외로 그대로 둔다).
- 의존명사(것, 수, 데, 만큼, 듯, 만 등)는 앞말과 띄어 쓴다 (예: "할것" → "할 것", "할수있다" → "할 수 있다", "만큼이나" 등 조사로 붙는 경우는 제외).
- 전체 콘텐츠를 한 번에 점검하면 토큰 비용이 크므로, 카테고리 단위로 나눠서 진행한다. 진행 시 카테고리 폴더당 파일 하나씩 병렬 에이전트로 처리하는 방식이 효율적이었음(2026-08-05 자작캠핑카 카테고리에서 검증).
